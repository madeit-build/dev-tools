...
- **Chunk 4c + Q&A BYOM shipped — OpenAI-compatible backends (BYOM complete):** a second `TourGenerator` implementation (`OpenAiAgentTourGenerator`) and a matching `StepAnswerer` OpenAI backend drive any OpenAI-wire-compatible endpoint (OpenAI, OpenRouter, Gemini-compat, local Ollama) via a shared `runOpenAiToolLoop`. Both backends (generation and Q&A) share the same provider-agnostic read-only tool layer (`read_file`/`grep`/`glob` + codemap `fileOutline`/`findSymbol`). Config: `hdtw.generation.provider` (`"anthropic"` | `"openai"`), `hdtw.generation.baseUrl`, `hdtw.generation.usdPer1kInput/Output` — this single setting now governs **both** tour generation and "Why?" detour Q&A. OpenAI key stored in SecretStorage → injected as `OPENAI_API_KEY` at engine spawn. The engine's anchor verification/repair/symbol-resolution pipeline is backend-blind. Spec: `docs/superpowers/specs/2026-06-15-qa-byom-design.md`. Remaining v1 gate: marketplace packaging (wasm bundling).
...
- **"Why?" detours shipped (Chunk 3b — V1 complete):** while walking any tour, the user may reply to the narration thread with a follow-up question. The engine handles `hdtw/askAboutStep`: a `StepAnswerer` port (Claude implementation: `ClauseStepAnswerer`; fake: `FakeStepAnswerer`, selected by `HDTW_GENERATOR=fake`) runs a read-only agent (Read/Grep/Glob, `maxTurns: 6`, budget-capped, cancellable via `AbortController` + JSON-RPC cancel). The VS Code `WalkController` appends the question + a "🧠 thinking…" placeholder comment immediately, then replaces it with the answer; on error (auth/budget/cancel/failure) the placeholder becomes a legible notice — the walk is never broken. The Q&A is ephemeral: navigating away or hitting Next disposes it. Auth/budget/cancel reuse the Chunk 2 guard layer; progress streams via the existing `hdtw/generationProgress` notification with a new `"answering"` phase. Design spec: `docs/superpowers/specs/2026-06-14-why-detours-design.md`.
- Not yet implemented: JetBrains client, release pipeline, code-map grounding (Chunk 4b), freshness CI (Chunk 5).
