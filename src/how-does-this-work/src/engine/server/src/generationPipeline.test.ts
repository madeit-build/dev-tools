...
test("verifyStep resolves a symbol-anchor to a range, fills the cache, keeps the symbol", async () => {
  ...
  expect(verified).toMatchObject({ anchor: { file: "a.ts", symbol: "alpha", startLine: 1, endLine: 3 } });
...
test("verifyStep still verifies a classic line-anchor", async () => {
  await writeFile(path.join(root, "a.ts"), ["line1", "line2", "line3"].join("\n"));
  const step = { title: "t", narration: "n", anchor: { file: "a.ts", startLine: 1, endLine: 2 } };
  const verified = await verifyStep(root, step);
  expect(verified).toMatchObject({ anchor: { file: "a.ts", startLine: 1, endLine: 2 } });
  expect((verified as { anchor: { symbol?: string } }).anchor.symbol).toBeUndefined();
});
