...
import {
  createCodemapMcpServer,
  runFileOutlineTool,
  runFindSymbolTool,
} from "./codemapTools.js";
...
test("fileOutline tool lists symbols; findSymbol returns a range; path traversal refused; absolute path refused; missing file returns friendly error", async () => {
  ...
  expect(
    (await runFindSymbolTool(root, "../x.ts", "alpha")).toLowerCase(),
  ).toContain("outside the workspace");
  expect(
    (await runFindSymbolTool(root, "/etc/passwd", "x")).toLowerCase(),
  ).toContain("outside the workspace");
  expect((await runFileOutlineTool(root, "ghost.ts")).toLowerCase()).toContain(
    "could not read",
  );
});
