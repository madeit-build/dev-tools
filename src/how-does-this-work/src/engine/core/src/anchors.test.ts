...
test("checkSymbolAnchorFreshness: unchanged = fresh, moved-verbatim or content-changed = relocated, no range = symbol-missing", () => {
  ...
  expect(
    checkSymbolAnchorFreshness(anchor, { startLine: 1, endLine: 3 }, file),
  ).toEqual({
    state: "fresh",
    startLine: 1,
    endLine: 3,
  ...
  const movedResult = checkSymbolAnchorFreshness(
    anchor,
  ...
  const editedResult = checkSymbolAnchorFreshness(
    anchor,
  ...
  expect(checkSymbolAnchorFreshness(anchor, undefined, file)).toEqual({
