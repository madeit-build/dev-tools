...
describe("scrubText: store paths in values, not just in provenance", () => {
  it("elides the hash from an ExecStart while keeping the derivation name", () => {
    expect(
      scrubText(
        "/nix/store/g9vyfyqc93dgr0vwz7lmnkyz0ni4yzpg-unit-script-wpa_supplicant-start/bin/x",
      ),
  ...
  it("leaves no literal /nix/store anywhere, which the artifact invariant requires", () => {
    expect(
      scrubText(
        "/nix/store/g9vyfyqc93dgr0vwz7lmnkyz0ni4yzpg-caddy-2.10.0/bin/caddy",
      ),
  ...
  it("scrubs a store path embedded mid-string, not only one at the start", () => {
    const out = scrubText(
      "sh -c /nix/store/g9vyfyqc93dgr0vwz7lmnkyz0ni4yzpg-thing/bin/go --flag",
    );
...
describe("scrubText: home paths with no repo-shaped tail", () => {
  ...
  it("strips a home path embedded mid-string", () => {
    expect(scrubText("--config /Users/someone/x.toml")).toBe(
      "--config ~/x.toml",
    );
  });
...
describe("secret-file values in any position", () => {
  ...
  it("redacts systemd's optional-file form, which carries a leading dash", () => {
    expect(
      sanitizeGraph(withAttr("-/run/secrets/rendered/x.env")).nodes[0].attrs.EnvironmentFile,
    ).toBe(REDACTED);
  });

  it("redacts a secret path that is not at the start of the value", () => {
    expect(
      sanitizeGraph(withAttr("--env-file /run/secrets/x.env")).nodes[0].attrs.EnvironmentFile,
    ).toBe(REDACTED);
  });
  ...
  it("redacts a systemd LoadCredential path, not only a sops-nix one", () => {
    expect(
      sanitizeGraph(withAttr("/run/credentials/x.service/db_password")).nodes[0]
        .attrs.EnvironmentFile,
    ).toBe(REDACTED);
  });
});
...
describe("edges and the ledger get sanitized, not only nodes", () => {
    ...
    edges: [
      {
        ...
        source: "inferred",
        evidence:
          "EXEC=/nix/store/g9vyfyqc93dgr0vwz7lmnkyz0ni4yzpg-thing/bin/x",
      },
    ],
    ledger: [
      {
  ...
  it("redacts evidence whose key half is secret-shaped", () => {
    const secretEvidence: Graph = {
      ...
      edges: [
        {
          ...g.edges[0],
          evidence: "DATABASE_URL=postgres://u:hunter2@host/db",
        },
      ],
    };
  ...
  it("keeps ordinary evidence readable, since evidence is the point of an inferred edge", () => {
    const plain: Graph = {
      ...g,
      edges: [{ ...g.edges[0], evidence: "reverse_proxy 127.0.0.1:11434" }],
    };
    expect(sanitizeGraph(plain).edges[0].evidence).toBe(
      ...
      nodes: [
        {
          ...
          host: "box",
          attrs: {
            exec: "/nix/store/g9vyfyqc93dgr0vwz7lmnkyz0ni4yzpg-a/bin/a",
          },
          provenance: {
            files: [
              "/nix/store/g9vyfyqc93dgr0vwz7lmnkyz0ni4yzpg-source/nix/x.nix",
            ],
          },
        },
      ],
