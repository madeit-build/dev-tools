...
describe("toRepoPath", () => {
  it("strips a nix store prefix down to the repo-relative path", () => {
    expect(toRepoPath("/nix/store/7ja3qcq17d764r430vn10x6nwlg6ihvb-source/nix/nixos/chat.nix"))
      .toBe("nix/nixos/chat.nix");
  });

  it("handles a store path whose name is not 'source'", () => {
    expect(toRepoPath("/nix/store/3n4qphl9s728sz8frmpqqrv9b1m87g68-my-flake/nix/flake.nix"))
      .toBe("nix/flake.nix");
  });
  ...
  it("keeps a runtime path that is not a store path, since it is a real fact", () => {
    expect(toRepoPath("/run/secrets/rendered/reverie.env")).toBe("/run/secrets/rendered/reverie.env");
  });

  it("strips a home directory prefix so no local structure survives", () => {
    expect(toRepoPath("/Users/someone/code/made-it/box.provisioning/nix/flake.nix"))
      .toBe("nix/flake.nix");
  });
});
...
describe("isSecretName", () => {
  it("flags the usual secret-shaped env var names", () => {
    for (const n of [
      "ANTHROPIC_API_KEY", "AWS_SECRET_ACCESS_KEY", "DB_PASSWORD",
      "GITHUB_TOKEN", "SESSION_SECRET", "private_key", "AUTH_CREDENTIAL",
    ]) {
  ...
  it("does not flag ordinary configuration names", () => {
    for (const n of ["PATH", "HOME", "OLLAMA_HOST", "OLLAMA_MODELS", "PORT", "TZ"]) {
...
const graphWith = (attrs: Record<string, string>): Graph => ({
  ...
  flakeRef: ".",
  nodes: [{
    id: "service:box/reverie", type: "service", label: "reverie", host: "box",
    attrs, provenance: { files: ["/nix/store/7ja3qcq17d764r430vn10x6nwlg6ihvb-source/nix/nixos/reverie.nix"] },
  }],
  edges: [],
  ledger: [],
});
...
describe("sanitizeGraph", () => {
  it("redacts the value of a secret-named attribute but keeps its key", () => {
    const out = sanitizeGraph(graphWith({ ANTHROPIC_API_KEY: "sk-ant-real-value-here" }));
  ...
  it("redacts any value that looks like it came out of /run/secrets", () => {
    const out = sanitizeGraph(graphWith({ EnvironmentFile: "/run/secrets/rendered/reverie.env" }));
