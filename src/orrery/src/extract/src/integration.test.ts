...
describe("integration against the fixture flake", () => {
  ...
  it("knows whether nix is available, and says so rather than skipping silently", () => {
    ...
    if (!available) {
      console.warn(
        "[orrery] nix not on PATH: the tier-2 integration suite did not run",
      );
    }
  });
  ...
  it.runIf(available)(
    ...
    () => {
      ...
      const alphaEdges = graph.edges.filter(
        (e) =>
          e.type === "declared-by" && e.from === "vhost:testbox/alpha.test",
      );
  ...
  it.runIf(available)(
    "splits a space-separated StateDirectory into two datastores",
    () => {
      const dirs = graph.nodes
        .filter(
          (n) =>
            n.type === "datastore"
            && n.id.startsWith("datastore:testbox/beta-app/"),
        )
        .map((n) => n.label)
        .sort();
  ...
  it.runIf(available)("hangs the fourth level off its service", () => {
    const e = graph.edges.find(
      (x) =>
        x.type === "contains" && x.to === "datastore:testbox/alpha-app/alpha",
    );
  ...
  it.runIf(available)(
    "accounts for every candidate in the ledger or the graph",
    () => {
      ...
      expect(
        graph.ledger.filter((r) => r.reason === "eval-failed"),
      ).toHaveLength(0);
    },
  );
