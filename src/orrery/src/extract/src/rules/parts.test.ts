...
describe("partsRule", () => {
  ...
  it("labels an ordinary env file plainly, not as a secret", () => {
    const r = partsRule(
      "box",
      { caddy: { state: null, workdir: null, envFile: "/etc/caddy/env" } },
      units,
      new Map(),
    );
  ...
  it("carries the port's own confidence, so a guessed port says so", () => {
    const guessed = new Map([
      [9999, { unit: "caddy", source: "inferred" as const }],
    ]);
    const r = partsRule(
      "box",
      { caddy: { state: null, workdir: null, envFile: null } },
    ...
    expect(r.nodes.find((n) => n.type === "port")?.attrs.confidence).toBe(
      "inferred",
    );
  });
  ...
  describe("against the real captured fleet", () => {
    ...
    it("handles every shape StateDirectory takes without throwing", () => {
      expect(() =>
        partsRule("box", fixture, allUnits, new Map()),
      ).not.toThrow();
    });
    ...
    it("finds caddy's state directory, which arrives as a list", () => {
      const r = partsRule("box", fixture, allUnits, new Map());
      expect(r.nodes.some((n) => n.id === "datastore:box/caddy/caddy")).toBe(
        true,
      );
    });
    ...
    it("produces no duplicate node ids across the whole fleet", () => {
      const ids = partsRule("box", fixture, allUnits, new Map()).nodes.map(
        (n) => n.id,
      );
    ...
    it("gives every node it emits a containing edge, so none is orphaned", () => {
      ...
      for (const n of r.nodes) {
        expect(
          r.edges.some((e) => e.type === "contains" && e.to === n.id),
          n.id,
        ).toBe(true);
      }
    });
