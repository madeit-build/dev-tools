...
import { buildPortIndex, parseEndpoints, linksRule, capAttr, MAX_ATTR_CHARS } from "./ports";
...
describe("parseEndpoints", () => {
  ...
  it("reads a port out of a URL that carries a path and a query", () => {
    expect(parseEndpoints("http://127.0.0.1:31088/search?q=<query>")).toEqual([31088]);
  });
...
describe("buildPortIndex", () => {
  it("maps a docker published port to its unit and marks it declared", () => {
    ...
    expect(idx.get(3000)).toEqual({ unit: "docker-grafana", source: "declared" });
  });
  ...
  it("falls back to an env var and marks that one inferred", () => {
    const idx = buildPortIndex({}, {}, { ollama: { OLLAMA_HOST: "127.0.0.1:11434" } });
  ...
  it("prefers a declared mapping over an inferred one for the same port", () => {
    const idx = buildPortIndex(
      { grafana: ["127.0.0.1:3000:3000"] }, {}, { something: { X_HOST: "127.0.0.1:3000" } },
    );
    expect(idx.get(3000)).toEqual({ unit: "docker-grafana", source: "declared" });
  });
...
describe("linksRule", () => {
  ...
  const idx = new Map([[11434, { unit: "ollama", source: "declared" as const }]]);

  it("draws a depends-on edge from a service to the service its env names", () => {
    const r = linksRule("box", { "open-webui": { OLLAMA_BASE_URL: "http://127.0.0.1:11434" } }, idx, units);
    const dep = r.edges.filter((e) => e.type === "depends-on");
    expect(dep).toHaveLength(1);
    expect(dep[0]).toMatchObject({
      from: "service:box/open-webui", to: "service:box/ollama", source: "inferred",
    });
  ...
  it("never draws a service depending on itself", () => {
    const selfIdx = new Map([[11434, { unit: "ollama", source: "declared" as const }]]);
    const r = linksRule("box", { ollama: { OLLAMA_HOST: "127.0.0.1:11434" } }, selfIdx, units);
    expect(r.edges).toHaveLength(0);
  });

  it("records an unresolvable endpoint in the ledger", () => {
    const r = linksRule("box", { "open-webui": { X_URL: "http://127.0.0.1:65000" } }, idx, units);
    expect(r.ledger[0].detail).toMatch(/65000/);
  });

  it("does not read an endpoint out of a secret-named variable", () => {
    const r = linksRule("box", { "open-webui": { API_KEY: "http://127.0.0.1:11434" } }, idx, units);
    expect(r.edges).toHaveLength(0);
  });
  ...
  describe("against the real captured fleet", () => {
    it("finds the service-to-service graph nothing else in the config states", () => {
      ...
      const from = (unit: string) =>
        r.edges.filter((e) => e.from === `service:box/${unit}`).map((e) => e.to);
