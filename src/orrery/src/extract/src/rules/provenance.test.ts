...
describe("provenanceRule", () => {
  it("emits one module node per contributing file", () => {
    const defs: RawDefinition[] = [{ file: `${STORE}/chat.nix`, names: ["chat.example"] }];
    const r = provenanceRule("box", "services.caddy.virtualHosts", defs, resolve);
    expect(r.nodes.filter((n) => n.type === "module")).toHaveLength(1);
    expect(r.nodes.some((n) => n.id === "module:nix/nixos/chat.nix")).toBe(true);
  });
  ...
  it("attributes each name to its own file rather than to all of them", () => {
    ...
    const r = provenanceRule("box", "services.caddy.virtualHosts", defs, resolve);
    const byFrom = new Map(r.edges.filter((e) => e.type === "declared-by").map((e) => [e.from, e.to]));
  ...
  it("gives one file that defines several names one edge per name", () => {
    const defs: RawDefinition[] = [
      { file: `${STORE}/observability.nix`, names: ["a.example", "b.example", "c.example"] },
    ];
    const r = provenanceRule("box", "services.caddy.virtualHosts", defs, resolve);
  ...
  it("emits an option node and a defines edge from each module to it", () => {
    ...
    expect(r.edges.filter((e) => e.type === "defines" && e.to === opt!.id)).toHaveLength(2);
  });

  it("emits one module node even when a file appears in several definitions", () => {
    const f = `${STORE}/a.nix`;
    const r = provenanceRule("box", "services.caddy.virtualHosts", [
  ...
  it("records a name with no node in the ledger rather than dangling an edge", () => {
    const defs: RawDefinition[] = [{ file: `${STORE}/a.nix`, names: ["ghost.example"] }];
    const r = provenanceRule("box", "services.caddy.virtualHosts", defs, () => null);
  ...
  describe("against the real captured fleet", () => {
    it("finds the eight modules that merge into one option", () => {
      const r = provenanceRule("box", "services.caddy.virtualHosts", fixture, resolve);
    ...
    it("attributes the five observability vhosts to observability.nix alone", () => {
      ...
      const fromObs = r.edges
        .filter((e) => e.type === "declared-by" && e.to === "module:nix/nixos/observability.nix")
        .map((e) => e.from)
        .sort();
    ...
    it("attributes the health responder to services.nix", () => {
      const r = provenanceRule("box", "services.caddy.virtualHosts", fixture, resolve);
      const edge = r.edges.find((e) => e.from === vhostId("box", "health.keep.madeit.build"));
      expect(edge?.to).toBe("module:nix/nixos/services.nix");
    });

    it("attributes chat to chat.nix, the single-vhost case", () => {
      const r = provenanceRule("box", "services.caddy.virtualHosts", fixture, resolve);
      const edge = r.edges.find((e) => e.from === vhostId("box", "chat.keep.madeit.build"));
