...
test("returns the final assistant text string", async () => {
  ...
  const result = await runOpenAiToolLoop(client, "gpt-test", "sys", "user", { maxTurns: 5, phase: "exploring", progressMessage: "x", workspaceRoot: root }, hooks());
...
test("maps a 401 to AuthRequiredError", async () => {
  ...
  const client: ChatClient = { chat: { completions: { create } } };
  await expect(
    runOpenAiToolLoop(client, "gpt-test", "sys", "user", { maxTurns: 5, phase: "exploring", progressMessage: "x", workspaceRoot: root }, hooks())
...
test("gives up with a GenerationFailedError after maxTurns of tool calls", async () => {
  ...
  const client: ChatClient = { chat: { completions: { create } } };
  await expect(
    runOpenAiToolLoop(client, "gpt-test", "sys", "user", { maxTurns: 3, phase: "exploring", progressMessage: "x", workspaceRoot: root }, hooks())
