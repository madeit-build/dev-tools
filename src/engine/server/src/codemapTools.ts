...
function guardPath(workspaceRoot: string, file: string): string | undefined {
...
export async function runFileOutlineTool(workspaceRoot: string, file: string): Promise<string> {
  const resolved = guardPath(workspaceRoot, file);
  ...
  return symbols
    .map((s) => `${s.qualifiedName} (${s.kind}) lines ${s.startLine}-${s.endLine}`)
    .join("\n");
}
