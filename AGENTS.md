...
- **Chunk 4c shipped — OpenAI-compatible generation backend (BYOM):** a second `TourGenerator` implementation (`OpenAiAgentTourGenerator`) drives any OpenAI-wire-compatible endpoint (OpenAI, OpenRouter, Gemini-compat, local Ollama) via a manual tool-calling explore loop. Both backends share the same provider-agnostic read-only tool layer (`read_file`/`grep`/`glob` + codemap `fileOutline`/`findSymbol`). Config: `hdtw.generation.provider` (`"anthropic"` | `"openai"`), `hdtw.generation.baseUrl`, `hdtw.generation.usdPer1kInput/Output`. OpenAI key stored in SecretStorage → injected as `OPENAI_API_KEY` at engine spawn. The engine's anchor verification/repair/symbol-resolution pipeline is backend-blind — the `TourGenerator` port absorbed the new backend with zero protocol or client changes. Q&A BYOM (`StepAnswerer`) remains pending.
