...
export function probeHunk(
  ...
  while (endIndex + 1 < lines.length) {
    ...
    endIndex++;
    if (indentOf(next) === contIndent && startsWithToken(next, tokens))
      hasToken = true;
  }

  if (!hasToken) return { kind: "skip" };
  if (contIndent <= indentOf(head))
    return { kind: "reject", reason: "bad-indent" };
...
const canon = (source: string) =>
  prettier.format(source, { ...base, printWidth: 9999 });
...
`--write` runs Prettier with the plugin, so its output is identical to what an editor produces. `--explain` runs Prettier _without_ the plugin and calls `hangAlign` directly, which yields the same decisions because Task 5 pinned that equivalence.
...
async function explain(files: string[]): Promise<void> {
  for (const file of files) {
    ...
    const { decisions } = hangAlign(plain, createAdapter(file), {
      printWidth,
      hangWidth:
        (options as { hangWidth?: number }).hangWidth ?? printWidth + 20,
...
export async function collectChecks(root: string): Promise<Check[]> {
  ...
  let support: Awaited<ReturnType<typeof prettier.getSupportInfo>> | null =
    null;
  try {
  ...
  const optionNames = new Set(
    (support?.options ?? []).map((option) => option.name),
  );
  ...
  return [
    ...
    },
    {
      ...
      detail: optionNames.has("experimentalOperatorPosition")
        ? "option present"
        : "option absent",
...
ignores whitespace _within_ a line comparison. A correct hang produces a
