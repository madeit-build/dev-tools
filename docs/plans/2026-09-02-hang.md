...
- **Every relative import carries an explicit `.js` extension**, including in test files, and the tsconfigs inherit `Node16` resolution from the base rather than overriding it. Node's ESM loader requires the extension. Without it `vitest` and `tsc` both pass and only `node dist/main.js` fails, which is the worst place to find out.
...
export { createAdapter, CONTINUATION_TOKENS } from "./adapter.js";
...
The root `package.json` must also depend on the plugin, or pnpm never links it
into the root `node_modules` and Prettier cannot resolve the name it is given.
...
Confirm the link exists before going further, because every later step in this
task depends on it:

```bash
ls -d node_modules/@made-i-t/hang-prettier
```
...
`git diff -w` does NOT work for this and must not be used. Hanging joins a
continuation line onto its head, so tokens move between lines, and `-w` only
ignores whitespace *within* a line comparison. A correct hang produces a
non-empty `-w` diff. Verified.

Use the same principle the differential oracle uses instead: reprint both
versions at a huge width so layout collapses, and compare. Only a real meaning
change survives that.
...
while IFS= read -r f; do
  [ -n "$f" ] || continue
  before=$(git show "HEAD:$f" | npx --no-install prettier --no-config --stdin-filepath "$f" --print-width 9999 --object-wrap collapse 2>/dev/null)
  after=$(npx --no-install prettier --no-config --stdin-filepath "$f" --print-width 9999 --object-wrap collapse < "$f" 2>/dev/null)
  if [ "$before" != "$after" ]; then
    echo "MEANING CHANGED: $f"
    status=1
  fi
done < <(git diff --name-only --diff-filter=M -- '*.ts' '*.tsx' '*.js' '*.mjs' '*.cjs')
[ "$status" -eq 0 ] && echo "all modified files: meaning preserved"
exit $status
```

`--no-config` is load-bearing: it keeps the hang plugin out of the comparison so
both sides are canonicalised by plain Prettier.

`--object-wrap collapse` on both invocations is also load-bearing. Prettier 3.5
defaults `objectWrap` to `"preserve"`, so reprinting at `--print-width 9999`
does NOT canonicalise an object literal's wrapping the way it canonicalises
everything else: it keeps whatever wrapping the input already had. Without
this flag, an expanded object literal and its collapsed form reprint to
different text even though nothing about the file's meaning changed, which
produced 73 false `MEANING CHANGED` hits during this dogfood. `--object-wrap
collapse` forces both sides to the same wrapping so the comparison is a real
meaning check again.

Expected: exit 0 and "all modified files: meaning preserved". Any `MEANING
CHANGED` line is a corruption bug. If one appears, revert the working tree, keep
the offending file as a refusal fixture, and fix the guard before continuing.
Do not proceed past this step with a failure.
...
Expect two distinct kinds of change mixed together, and separate them as you
read. Setting `experimentalOperatorPosition` moves operators to the start of
continuation lines across the whole repo, which is Prettier's doing and happens
whether or not this tool exists. Hanging alignment is this tool's doing. Both
are meaning-preserving and both pass step 7, so only reading tells them apart.
