...
while IFS= read -r f; do
  [ -n "$f" ] || continue
  before=$(git show "HEAD:$f" | npx --no-install prettier --no-config --stdin-filepath "$f" --print-width 9999 2>/dev/null)
  after=$(npx --no-install prettier --no-config --stdin-filepath "$f" --print-width 9999 < "$f" 2>/dev/null)
