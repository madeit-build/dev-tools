...
- **Never log or serialize a secret value.** Env var *names* and secret file *paths* are architectural facts and are kept. Values are not, ever.
...
describe("buildPortIndex", () => {
  ...
  it("prefers a declared mapping over an inferred one for the same port", () => {
    const idx = buildPortIndex(
      { grafana: ["127.0.0.1:3000:3000"] }, {}, { something: { X: "127.0.0.1:3000" } },
    );
    expect(idx.get(3000)).toEqual({ unit: "docker-grafana", source: "declared" });
  });
...
```typescript
import { hostId, serviceId, vhostId, type OrreryEdge, type OrreryNode, type DropRecord } from "@made-i-t/orrery-model";
import type { RuleResult } from "./services";
...
describe("provenanceRule", () => {
  it("emits one module node per contributing file", () => {
    const defs: RawDefinition[] = [
      { file: "/nix/store/7ja3qcq17d764r430vn10x6nwlg6ihvb-source/nix/nixos/chat.nix", names: ["chat.example"] },
    ];
  ...
  it("attributes each name to its own file rather than to all of them", () => {
    const defs: RawDefinition[] = [
      { file: "/nix/store/7ja3qcq17d764r430vn10x6nwlg6ihvb-source/nix/nixos/a.nix", names: ["one.example"] },
      { file: "/nix/store/7ja3qcq17d764r430vn10x6nwlg6ihvb-source/nix/nixos/b.nix", names: ["two.example"] },
    ];
    const r = provenanceRule("box", "services.caddy.virtualHosts", defs, resolve);
  ...
  it("emits one module node even when a file appears in several definitions", () => {
    const f = "/nix/store/7ja3qcq17d764r430vn10x6nwlg6ihvb-source/nix/nixos/a.nix";
...
```typescript
import {
  fleetId, hostId, serviceId, vhostId, sanitizeGraph, validateGraph,
  type DropRecord, type Graph, type OrreryEdge, type OrreryNode,
} from "@made-i-t/orrery-model";
import { nixEval, nixEvalOptional, flakeMetadata, type FlakeMetadata } from "./nix";
...
export function parseArgs(argv: string[]): Args {
  ...
  return {
    ...
    flakeRef: (positional[0] === "doctor" ? positional[1] : positional[0]) ?? ".",
    out: flag("out") ?? "out",
  };
}
  ...
  return [
    {
      ...
      run: async () => {
        ...
        return v
          ? { ok: true, detail: v }
          : { ok: false, detail: "nix not found", fix: "Install Nix, or add it to PATH. On macOS the daemon installer puts it in /nix/var/nix/profiles/default/bin." };
      },
    },
    {
      name: "flake resolves",
...
.orrery-node__label { color: var(--ink); font-weight: 600; }
.orrery-node__meta { color: var(--muted-soft); font-size: 11px; }

/* Host and fleet read as containers: heavier border, flatter fill. */
.orrery-node--host, .orrery-node--fleet { border-width: 2px; background: var(--surface-control); }

/* A vhost is the front door, so it gets the inverted ground. */
.orrery-node--vhost { background: var(--ink); border-color: var(--ink); }
.orrery-node--vhost .orrery-node__label { color: var(--ink-inverse); }
.orrery-node--vhost .orrery-node__meta { color: var(--muted-inverse); }

/* A module is source, not runtime: dashed border says "this is not a thing
   that runs". */
.orrery-node--module { border-style: dashed; background: transparent; }

/* A oneshot job is present but subordinate to the services around it. */
.orrery-node--job { border-color: var(--border); }
.orrery-node--job .orrery-node__label { color: var(--muted); font-weight: 400; }

.orrery-node--selected { outline: 2px solid var(--ink); outline-offset: 2px; }
...
.orrery-edge--inferred .react-flow__edge-path { stroke-dasharray: 5 4; stroke: var(--muted-soft); }
```
...
```typescript
import type { Graph, EdgeType, NodeType, OrreryEdge, OrreryNode } from "@made-i-t/orrery-model";
...
export function visibleFor(
  ...
  if (nodes.length === 0 && view.path.length > 1) {
    nodes = childrenOf(graph, `vhost:${view.path[0]}/${view.path.slice(1).join("/")}`);
  }
...
describe("partsRule", () => {
  ...
  describe("against the real captured fleet", () => {
    it("handles every shape StateDirectory takes without throwing", () => {
      expect(() => partsRule("box", fixture, new Set(Object.keys(fixture)), new Map())).not.toThrow();
    });

    it("finds both of postgres's state directories, not just the first", () => {
      const r = partsRule("box", fixture, new Set(Object.keys(fixture)), new Map());
    ...
    it("produces no duplicate node ids across the whole fleet", () => {
      const ids = partsRule("box", fixture, new Set(Object.keys(fixture)), new Map()).nodes.map((n) => n.id);
      expect(new Set(ids).size).toBe(ids.length);
    });
  });
});
...
import {
  ...
  type OrreryEdge, type OrreryNode,
} from "@made-i-t/orrery-model";
import type { RuleResult } from "./services";
...
export function partsRule(
  ...
  const add = (node: OrreryNode, from: string, type: OrreryEdge["type"]): void => {
    if (!seen.has(node.id)) {
      seen.add(node.id);
      nodes.push(node);
      edges.push({
...
```typescript
    if (attr.endsWith("config.systemd.services") && apply?.includes("StateDirectory")) return parts;
```
...
```typescript
  it("reaches the fourth zoom level, so drilling into a service is not empty", async () => {
    const g = await buildGraph(".", deps);
    const parts = g.nodes.filter((n) => n.type === "datastore" || n.type === "port");
...
Expected: `0`. The secret *paths* must be redacted in `attrs` while the nodes labeled "secret env" remain. Confirm both:
...
describe("integration against the fixture flake", () => {
  ...
  it.runIf(available)("does not attribute a vhost to a module that did not declare it", () => {
    const wrong = graph.edges.find(
      (e) => e.type === "declared-by" && e.from === "vhost:testbox/alpha.test" && e.to === "module:beta.nix",
    );
